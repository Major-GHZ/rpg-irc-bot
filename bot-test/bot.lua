...
function IRCClient:handle_005(msg)
    local params = {}
    for _, arg in ipairs(msg.args) do
    ...
    if params.MODES then
        self._maxmodes = tonumber(params.MODES)
    end
    if params.PREFIX then
        local m = params.PREFIX:match("^%((.+)%)(.+)$")
        if m then
            for i = 1, #m[2] do
                self._prefixmodes[m[2]:sub(i, i)] = m[1]:sub(i, i)
            end
        end
    end
    if params.CHANMODES then
        local m = params.CHANMODES:match("^([^,]+),([^,]+),([^,]+),?(.*)$")
        if m then
            for i = 1, #m[1] do
                self._modetypes[m[1]:sub(i, i)] = 1
            end
            for i = 1, #m[2] do
                self._modetypes[m[2]:sub(i, i)] = 2
            end
            for i = 1, #m[3] do
                self._modetypes[m[3]:sub(i, i)] = 3
            end
            if m[4] then
                for i = 1, #m[4] do
                    self._modetypes[m[4]:sub(i, i)] = 4
                end
            end
        end
    end
...
function IRCClient:handle_376(msg)
    self:mode(conf.get("botnick"), conf.get("botmodes"))
    self:join(conf.get("botchan"))
end

function IRCClient:handle_422(msg)
    self:mode(conf.get("botnick"), conf.get("botmodes"))
    self:join(conf.get("botchan"))
end

function IRCClient:handle_join(msg)
    self:add_user(msg.src, msg.src .. "!" .. msg.user .. "@" .. msg.host, {}, msg.time)
end

function IRCClient:handle_part(msg)
    local user = self:remove_user(msg.src)
    self._bot:on_nick_parted(user)
end

function IRCClient:handle_kick(msg)
    local user = self:remove_user(msg.args[1])
    self._bot:on_nick_kicked(user)
end

function IRCClient:handle_mode(msg)
    if msg.args[0] ~= conf.get("botchan") then
        return
    end
    local changes = {}
    local params = {}
    for i = 2, #msg.args do
        local m = msg.args[i]:match("^([+-])(.*)$")
        if m then
            for j = 1, #m[2] do
                table.insert(changes, { sign = m[1], mode = m[2]:sub(j, j) })
            end
        else
            table.insert(params, msg.args[i])
        end
    end
    for _, change in ipairs(changes) do
        local modetype = self._modetypes[change.mode]
        if modetype == 1 or modetype == 2 or (modetype == 3 and change.sign == '+') then
            local param = table.remove(params, 1)
            if modetype == 2 then
                if change.sign == '+' then
                    if self._users[param] then
                        self._users[param].modes[change.mode] = true
                    end
                else
                    if self._users[param] then
                        self._users[param].modes[change.mode] = nil
                    end
                end
            end
        end
...
function IRCClient:handle_nick(msg)
    if self._users[msg.src] then
        self._bot:on_nick_changed(self._users[msg.src], msg.args[0])
        self._users[msg.args[0]] = self._users[msg.src]
        self._users[msg.args[0]].nick = msg.args[0]
        self._users[msg.src] = nil
    end
    if msg.src == self._nick then
        self._nick = msg.args[0]
    end
end

function IRCClient:handle_quit(msg)
    local user = self:remove_user(msg.src)
    self._bot:on_nick_quit(user)
end

function IRCClient:handle_privmsg(msg)
    if msg.args[0] == self._nick then
        self._bot:on_private_message(self._users[msg.src], msg.trailing)
    else
        self._bot:on_channel_message(self._users[msg.src], msg.trailing)
    end
end
